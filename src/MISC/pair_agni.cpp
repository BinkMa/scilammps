...
void PairAGNI::compute(int eflag, int vflag)
{
  ...
  outputFile2.open("valueX.txt",ios::app);
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    //if( clock_gettime(CLOCK_REALTIME, &start2) == -1) { perror("clock gettime");}
	...
	distance.push_back(r);
        ...
        //printf("distance r:%lf\n",r);
	//if( clock_gettime(CLOCK_REALTIME, &start1) == -1) { perror("clock gettime");}
    ...
    //if( clock_gettime( CLOCK_REALTIME, &stop2) == -1 ) { perror("clock gettime");}
    //time2 = (stop2.tv_sec - start2.tv_sec)+ (double)(stop2.tv_nsec - start2.tv_nsec)/1e9;
    //printf("iteration  for t2 is %d \n",jnum);
    //printf(" Execution time t2= %f sec,\n", time2);
    
    copy(distance.begin(), distance.end(), ostream_iterator<float>(outputFile , " "));
    copy(xx.begin(), xx.end(), ostream_iterator<float>(outputFile2 , " "));
    outputFile << endl;
    outputFile2 << endl;


    //if( clock_gettime(CLOCK_REALTIME, &start3) == -1) { perror("clock gettime");}
    ...
    /*
    if( clock_gettime(CLOCK_REALTIME, &stop3) == -1) { perror("clock gettime");}
    time2 = (stop3.tv_sec - start3.tv_sec)+ (double)(stop3.tv_nsec - start3.tv_nsec)/1e9;
    printf("iteration  for t3 is %d \n",iparam.numtrain);
    printf(" Execution time t3 = %f sec,\n", time2);
*/
